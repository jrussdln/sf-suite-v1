...
<section class="container" id="personnalInfoSection">
  ...
  <form id="userForm" action="../api/api_tracer.php" method="POST" autocomplete="off">
...
$(document).ready(function () {
    $('#userForm').on('submit', function (e) {
        ...
        const identifier = $('#uap_identifier').val().trim();
        ...
        // Optional: Log data to verify it's serializing properly
        const formData = $(this).serialize();
        console.log('Form Data:', formData);

        $.ajax({
            url: '../api/api_tracer.php',
            type: 'POST',
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    $('#continueBtn').show(); // Show continue button on success
                } else {
    ...
    fetch('fetch_user.php', {
      ...
      .then(data => {
        if (data.success) {
          ...
          document.getElementById('uap_identifier').disabled = false;
        } else {

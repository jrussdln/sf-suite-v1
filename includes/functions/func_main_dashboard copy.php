...
function calculateEnrollmentChange($current, $previous)
{
    if ($previous == 0 && $current == 0) {
        ...
        return ['value' => 100, 'direction' => 'increased'];
    }
    ...
    if ($current > $previous) {
        return ['value' => round($percentage, 2), 'direction' => 'increased'];
    } elseif ($current < $previous) {
        return ['value' => round($percentage, 2), 'direction' => 'decreased'];
    } else {
        return ['value' => 0, 'direction' => 'no change'];
    }
}
...
function get_section_attendance($pdo)
{
    try {
        ...
        // Fetch all sections
        $sectionsStmt = $pdo->prepare("SELECT TRIM(LOWER(SectionName)) AS SectionName FROM section_tbl WHERE SchoolYear = :syTerm");
        ...
        // Query to get attendance counts and "X" counts
        $query = "
            SELECT  
            ...
            LEFT JOIN attendance_tbl a 
                ON TRIM(LOWER(s.SectionName)) = TRIM(LOWER(a.section)) 
        ...
        foreach ($attendanceData as $row) {
            ...
            $attendanceMap[$row['SectionName']] = [
                'attendance_count' => $attendanceCount,
                'x_count' => $xCount,
                'attendance_percentage' => $attendancePercentage
            ];
        }
        // Ensure every section appears
        $finalResult = [];
        foreach ($sections as $section) {
            $sectionName = $section['SectionName'] ?? 'Unknown Section';
            $finalResult[] = [
                'SectionName' => $sectionName,
                'attendance_count' => $attendanceMap[$sectionName]['attendance_count'] ?? 0,
                'x_count' => $attendanceMap[$sectionName]['x_count'] ?? 0,
                'attendance_percentage' => $attendanceMap[$sectionName]['attendance_percentage'] ?? 0
            ];
        }
        return $finalResult;
    } catch (PDOException $e) {

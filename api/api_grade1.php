...
switch ($method) {
    case 'GET':
        if (isset($_GET['get_student_grade'])) {
            ...
            exit;
        } elseif (isset($_GET['get_subjects'])) {
            $grade_level = $_GET['grade_level'];
            $school_year = $_GET['school_year'];
            $query = "SELECT subject_code, subject_id FROM subjects_tbl WHERE grade_level = ? AND subject_term = ? ORDER BY subject_order ASC";
            $stmt = $pdo->prepare($query);
            $stmt->execute([$grade_level, $school_year]);
            $subjects = [];
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                $subjects[] = [
                    'subject_code' => $row['subject_code'],
                    'subject_id' => $row['subject_id'] // Include subject_id in the response
                ];
            }
